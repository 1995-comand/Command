...
export default function Chat() {
    ...
    useEffect(() => {
        ...
        const onMessageNew = (newMessage) => {
            ...
            // Show message if: active user is the sender OR active user is the receiver (and I am the other party)
            const isRelevant = activeId && (
                senderId === activeId ||
                (receiverId === activeId && senderId === myId) ||
                (senderId === myId && receiverId === activeId) ||
                (receiverId === myId && senderId === activeId)
            );
            if (isRelevant) appendMessageUnique(newMessage);
        };
    ...
    const VoiceMessage = ({ src, time, isMine, fileSize }) => {
        ...
        return (
            <div className="flex w-[280px] items-center gap-[10px]">
                <button type="button" onClick={togglePlay}
                    className="flex h-[46px] w-[46px] shrink-0 items-center justify-center rounded-full bg-[#49a8e8] text-[20px] text-white transition active:scale-95">
                    {isPlaying ? "⏸" : "▶"}
                </button>
                <div className="flex flex-1 flex-col min-w-0">
                    <div className="flex h-[24px] items-end gap-[2px] overflow-hidden">
                        {Array.from({ length: 40 }).map((_, i) => (
                            <span key={i}
                                className={`w-[2px] shrink-0 rounded-full transition-colors ${progress > 0 && i / 40 <= progress ? "bg-[#49a8e8]" : isMine ? "bg-[#5d8ab3]" : "bg-[#42607d]"}`}
                                style={{ height: `${4 + ((i * 13 + 7) % 16)}px` }}
                            />
                        ))}
                    </div>
                    <div className="mt-[4px] flex items-center justify-between gap-[8px]">
                        <span className="text-[12px] text-[#82a5c8]">
                            {formatAudioTime(shownTime)}
                        </span>
                        <span className="shrink-0 text-[11px] text-[#7f92a6] flex items-center gap-[3px]">
                            {time}
                            {isMine && <span className="text-[#49a8e8]">✓✓</span>}
                        </span>
                    </div>
                            ...
                            <div className="relative z-10 h-full overflow-y-auto pb-[90px] pt-[8px]">
                                ...
                                {loadingMessages ? (
                                    ...
                                    messages.map((m, index) => {
                                        ...
                                        // Telegram-style border radius based on grouping
                                        const bubbleRadius = isMine
                                            ? `rounded-tl-[18px] rounded-tr-[18px] rounded-bl-[18px] ${isLastInGroup ? "rounded-br-[4px]" : "rounded-br-[18px]"}`
                                            : `rounded-tr-[18px] rounded-bl-[18px] rounded-br-[18px] ${isLastInGroup ? "rounded-tl-[4px]" : "rounded-tl-[18px]"}`;

                                        const bubbleBg = isMine ? "bg-[#2b5278]" : "bg-[#182533]";
                                        ...
                                        return (
                                            <div key={m?._id || index}
                                                ...
                                                <div className="flex flex-col" style={{ maxWidth: "72%" }}>
                                                    ...
                                                    {/* Bubble */}
                                                    <div
                                                        className={`${bubbleBg} ${bubbleRadius} shadow-sm`}
                                                        style={{
                                                            maxWidth: isLocation ? "300px" : undefined,
                                                            padding: isLocation ? "0" : hasAudio ? "10px 12px" : "7px 12px 6px",
                                                            overflow: isLocation ? "hidden" : undefined,
                                                        }}
                                                    >
                                                        {isLocation ? (
                                                            <LocationMessage
                                                            ...
                                                            <>
                                                                <FileMessage msg={m} />
                                                                <div className="flex justify-end items-center gap-[4px] mt-[3px] px-[2px]">
                                                                    <span className="text-[11px] text-white/50">{timeStr}</span>
                                                                    {isMine && <span className="text-[11px] text-[#49a8e8]">✓✓</span>}
                                                                </div>
                                                            </>
                                                        ) : (
                                                            /* Text message — timestamp floats inline at bottom-right (Telegram style) */
                                                            <div className="relative">
                                                                <p className="break-words text-[15px] leading-[20px] text-white">
                                                                    {m?.text}
                                                                    {/* Invisible spacer pushes text away from timestamp */}
                                                                    <span className="invisible inline-flex items-center gap-[3px] text-[11px] ml-[6px] align-bottom whitespace-nowrap select-none">
                                                                        {timeStr}{isMine ? " ✓✓" : ""}
                                                                    </span>
                                                                </p>
                                                                <span className="absolute bottom-0 right-0 flex items-center gap-[3px] text-[11px] text-white/50 whitespace-nowrap leading-none">
                                                                    {timeStr}
                                                                    {isMine && <span className="text-[#49a8e8]">✓✓</span>}
                                                                </span>
                                                            </div>
                                ...
                                {typingText ? (
                                    <div className="mb-[8px] flex justify-start">
                                        <div className="rounded-tl-[4px] rounded-tr-[18px] rounded-bl-[18px] rounded-br-[18px] bg-[#182533] px-[14px] py-[10px] flex items-center gap-[4px]">
                                            {[0, 1, 2].map((i) => (
                                                <span key={i}
                                                    className="h-[7px] w-[7px] rounded-full bg-[#8b91a4]"
